...
from ...__arch_helpers import arch_util
from ...__arch_helpers.deform_conv import DeformConv2d
from ...__arch_helpers .ED_offset import Encoder_Decoder as MotionNet
...
class D_Align(nn.Module):
    def __init__(self, nf=64, groups=4):
        ...
        self.cas_dcnpack = DeformConv2d(nf, nf, 3, padding=1, bias=True)
    ...
    def forward(self, nbr_fea_l, ref_fea_l):
        ...
        L1_fea = self.lrelu(self.cas_dcnpack(nbr_fea_l, offset))  # Deform alignment for L1 aligned frames
        return L1_fea
...
class MFDIN_OLD2P(nn.Module):
    def __init__(self, nf=64, groups=4, front_RBs=5, back_RFAs=2, center=None, nfields=5):
        ...
        self.feature_extraction = arch_util.make_layer(
            ResidualBlock_noBN_f, front_RBs
        )  # Extract features with the front residual block group
        ...
        self.fusion = nn.Conv2d(nfields * nf, nf, 1, 1, bias=True)  # Normal fusion convolution function
        #### reconstruction
        self.recon_trunk = arch_util.make_layer(RFABlock, back_RFAs)  # Reconstruct image with the back residual block group
    ...
    def forward(self, x):
        ...
        f_fea = self.feature_extraction(fea1)  # Extract features with the front residual block group
        ...
        #### pcd align
        # ref feature list
        ref_fea_lA = f_fea[:, self.center, :, :, :].clone()  # Set the reference frame for each layer
        ...
        feaA = self.fusion(aligned_feaA)  # TSA fusion module
        feaB = self.fusion(aligned_feaB)
        fea = torch.stack((feaA, feaB), dim=1).view(B * 2, -1, H, W)
        out = self.recon_trunk(fea)  # Reconstruction module
