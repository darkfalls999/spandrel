...
#from scipy.interpolate import interp2d               Removed from SciPy 1.14.0
from scipy.interpolate import RegularGridInterpolator  #SciPy >= 1.14.0
import numpy as np
import torch
...
def shift_pixel(x: torch.Tensor, sf: int, upper_left: bool = True) -> torch.Tensor:
    """shift pixel for super-resolution with different scale factors
    Args:
        x: tensor image or kernel
        sf: scale factor
    ...
    yv, xv = np.arange(0, h, 1.0), np.arange(0, w, 1.0)
    ...
    Y, X = np.meshgrid(y1, x1, indexing='ij')
    points = np.stack([Y.ravel(), X.ravel()], axis=-1)

    if x.ndim == 2:
        # For 2D input (single channel)
        ...
        x_shifted = interpolator(points).reshape(h, w)
    else:
        # For 3D input (multi-channel)
        ...
        for i in range(x.shape[-1]):
            ...
            x_shifted[..., i] = interpolator(points).reshape(h, w)

    # Convert back to tensor if input was tensor
    if isinstance(x, torch.Tensor):
        x_shifted = torch.from_numpy(x_shifted).to(device)
...
def pad_circular(input: torch.Tensor, padding: list[int]) -> torch.Tensor:
    """
    Arguments
...
def dim_pad_circular(input: torch.Tensor, padding: int, dimension: int) -> torch.Tensor:
