...
def fspecial_gaussian(hsize: int, sigma: float):
    hsize = [hsize, hsize]
...
def fspecial_laplacian(alpha: float) -> np.ndarray:
    """Create a 3x3 Laplacian filter
    
    Args:
        alpha: Controls shape of Laplacian (range 0.0 to 1.0)
              Default is 0.2 for standard Laplacian approximation
    
    Returns:
        3x3 numpy array representing the Laplacian filter
    """
